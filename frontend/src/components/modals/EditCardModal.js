...
const EditCardModal = ({ card, list, setShowModal }) => {
    ...
    } = useAxiosGet(`/boards/attachments/?item=${card.id}`);

    
    const addAttachment = async(e) =>{
        await authAxios.post(`${backendUrl}/boards/attachments/`,{
            item: card.id,
            ...
            <div className="edit-modal__cols">
                <div className="edit-modal__left">
                    ...
                    <div className="edit-modal__section-header">
                        <div>
                            <i className="fal fa-list"></i> Task Completed
                        </div>
                        {card.card_completed ? (
                    ...
                    <div className="edit-modal__section-header">
                        <div>
                            <i className="fal fa-clock"></i> Due Date
                        </div>
                        <input
                            type="datetime-local"
                            defaultValue={card.due_date ? new Date(card.due_date).toISOString().substr(0, 10) : ""}
                        />
                    </div>
                    <div className="edit-modal__section-header">
                        <div>
                    ...
                    >
                        ...
                        <div>
                            <input type="file" className="btn btn--secondary btn--small attachment-btn" value={attachmentURL} onChange={addAttachment}/>
                    ...
                    <Attachments attachments={card.attachments} removeAttachment={removeAttachment} />
                    <CommentForm
                        card={card}
                        style={
                            (comments || []).length === 0
...
const Attachments = ({ attachments, removeAttachment}) => {
    ...
    const deleteAttachment = async (attachment) => {
        await authAxios.delete(`${backendUrl}/boards/attachments/${attachment.id}/`);
            ...
            {attachments.map((attachment) => (
                <li key={uuidv4()}>
                    <div className="attachment">
                        <img src={backendUrl + attachment.upload} />
                        <div className="attachment__content">
                            <div className="attachment__buttons">
                                <a className="btn btn--secondary btn--small" onClick={() => deleteAttachment(attachment)}>
                                    Delete
                                </a>
                            </div>
                        </div>
...
const CommentForm = ({
    ...
    // state for checking if comment is null
    ...
    return (
        <form
            className={`edit-modal__form${comment ? " edit-modal__form--comment" : ""
                }`}
            style={style}
